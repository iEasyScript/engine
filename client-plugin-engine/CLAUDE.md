...
```kotlin
// BAD
val v = com.projectx.game.math.Vector2f(1f, 2f)
val x = com.projectx.script.api.varps.getVarBit(123)
...
⛔ **Never run `buildNativeBootstrap` while the engine is injected.** The install step overwrites the
`.so` the client currently has mapped, which SIGSEGVs it instantly. Uninject → rebuild → reinject.
`compileKotlin` is safe at any time.

### Cross-building `projectxbootstrap.dll` from Linux

```bash
cd client-plugin-engine/native-bootstrap
cmake --preset windows-mingw -DJNI_INCLUDE_DIRS="<win-jdk>/include;<win-jdk>/include/win32"
cmake --build build-windows-mingw -j"$(nproc)"
```

`JNI_INCLUDE_DIRS` must be supplied explicitly and must point at a **Windows** JDK: `find_package(JNI)`
resolves the host JDK otherwise, and its `jni_md.h` defines `jint`/`jlong` for LP64 rather than LLP64.
The build uses the headers only — the JVM is loaded at runtime via `LoadLibraryW`, never linked.

The resulting DLL must depend on nothing outside a stock Windows install, since it is injected into a
client that knows nothing about MinGW. Verify after any link change:

```bash
x86_64-w64-mingw32-objdump -p build-windows-mingw/projectxbootstrap.dll | grep "DLL Name:" | sort -u
```

Anything matching `libgcc_s_seh-1` / `libstdc++-6` / `libwinpthread-1` means the static-runtime link
options were lost and `LoadLibraryW` will fail on the target machine.
...
### Core Modules (`src/main/kotlin/com/projectx/`)
...
- **`script/`** — coroutine-based scripting framework
  ...
  - Scripts auto-discovered from `~/.projectx/scripts/` via `@ScriptDescription` annotation
- **`pathfinder/`** — route finding with collision detection (`RouteFinder.kt`, `WorldCollision.kt`)
- **`traversal/`** — DSL for building movement/interaction sequences as linked `TraversalNode` chains. See `src/main/kotlin/com/projectx/traversal/README.md` for DSL docs
