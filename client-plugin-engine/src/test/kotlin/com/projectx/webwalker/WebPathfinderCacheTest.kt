...
class WebPathfinderCacheTest {
    ...
    fun `routes through doors`() {
        WebPathfinder().find(3222, 3218, 0, 3185, 3436, 2)
        ...
        for ((id, side) in doors) {
            val door = Tile(id)
            val result = WebPathfinder().find(door.x, door.y, 0, door.x + sides[side][0], door.y + sides[side][1], 0)
