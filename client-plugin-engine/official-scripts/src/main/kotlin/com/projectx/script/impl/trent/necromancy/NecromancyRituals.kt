package com.projectx.script.impl.trent.necromancy
...
private val NECROMANCY_POT_REGEX = Regex(".*((extreme|super) necromancy|necromancy potion).*", RegexOption.IGNORE_CASE)
    ...
    override suspend fun NecromancyRituals.stateLoop() {
        if (varps.getVarBit(RITUAL_ACTIVE_VARBIT) == 0) {
            ...
            if (findClosestNPC { it.name.contains("depleted") } != null) {
                if (inventory.hasItem(NECROMANCY_POT_REGEX)) {
