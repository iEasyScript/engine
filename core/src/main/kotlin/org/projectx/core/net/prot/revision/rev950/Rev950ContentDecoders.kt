 ...
 */
internal fun Codec.registerRev950ContentDecoders() {
    containers()
    worldBuild()
    zoneEvents()
}
...
private fun Codec.worldBuild() {
    ...
    content(
        ...
        source.skip(1L)
        int("type", source.readByteAdd())
        int("centerZoneX", source.readUShortLittle(), RefDomain.COORD)
...
private fun Source.readObjIdLittle(): Int = readUByte() or (readUByte() shl 8) or (readUByte() shl 16)
...
private fun Codec.zoneEvents() {
    content(
        ...
        val shapeFlags = source.readByteInverse() and 0xFF
        int("loc", source.readUIntMiddle(), RefDomain.LOC)
        source.skip(1L)
        val packed = source.readByteInverse() and 0xFF
    ...
    content(
        ...
        int("obj", source.readObjIdLittle(), RefDomain.OBJ)
        val packed = source.readByteSubtract() and 0xFF
    ...
    content(
        ...
        int("obj", source.readUMedium(), RefDomain.OBJ)
