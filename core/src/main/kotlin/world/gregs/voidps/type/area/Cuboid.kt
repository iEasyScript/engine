...
data class Cuboid(
    ...
    override fun toMapSquares(): List<MapSquare> {
        val list = mutableListOf<MapSquare>()
        val max = Tile(maxX, maxY, maxLevel).mapSquare
        val min = Tile(minX, minY, minLevel).mapSquare
        for (x in min.x..max.x) {
            for (y in min.y..max.y) {
                list.add(MapSquare(x, y))
            }
        }
        return list
    }
    ...
    fun toMapSquareLevels(): List<MapSquareLevel> {
        val list = mutableListOf<MapSquareLevel>()
        val max = Tile(maxX, maxY, maxLevel).mapSquareLevel
        val min = Tile(minX, minY, minLevel).mapSquareLevel
        for (level in min.level..max.level) {
            for (x in min.x..max.x) {
                for (y in min.y..max.y) {
                    list.add(MapSquareLevel(x, y, level))
                }
            }
        }
        return list
