...
Build the jar in the script repository (`./gradlew jar`) and copy it into `~/.projectx/scripts/`.
...
- This is **safe while the engine is injected** — it writes only the script jar, not the engine `.so`/shadowJar.
  ...
  use `./gradlew compileKotlin` in the script repository.)
